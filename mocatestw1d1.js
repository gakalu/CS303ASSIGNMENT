...
describe("reverseArray", function () {
    it("tests reverseArray odd number elements", function () {
        assert.strictEqual(test.reverseArray(["A", "B", "C"]), ["C", "B", "A"]);
    });
    it("tests reverseArray even number elements", function () {
        assert.strictEqual(test.reverseArrayInPlace(["A", "B", "C", "D"]), ["D", "C", "B", "A"]);
    });
    it("tests reverseArrayInPlace even number elements", function () {
        assert.strictEqual(reverseArrayInPlace(["A", "B", "C", "D"]), ["D", "C", "B", "A"]);
    });
    it("tests reverseArrayInPlace odd number elements", function () {
        assert.strictEqual(test.reverseArray([1, 2, 3, 4, 5]), [5, 4, 3, 2, 1]);
    });
});
...
describe("score exam", function () {
    ...
    it("exam with 3 students", function () {
        assert.strictEqual(test.scoreExams(studentAnswers, correctAnswers), [2, 2, 2]);
    });
});